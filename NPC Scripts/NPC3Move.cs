...
public class NPC3Move : TacticsMove
{
    ...
    {
        Debug.Log("Searching for Player NPC3");
        RaycastHit hit;
        // Does the ray intersect the player, does the enemy see player
        if (Physics.Raycast(transform.position, transform.TransformDirection(Vector3.forward), out hit, 2.0f) && hit.transform.CompareTag("Player"))
        {
